...
def train(opts):
    ...
    optimizer_D = torch.optim.Adam(
        discriminator.parameters(), lr=opts.lr, betas=(opts.b1, opts.b2)
    )
    ...
    for epoch in range(opts.init_epoch, opts.n_epochs + 1):
        for batch_idx, batch in enumerate(train_dataloader):
            ...
            batches_left = (opts.n_epochs - opts.init_epoch) * len(
                train_dataloader
            ) - batches_done
            ...
            if batches_done % opts.sample_freq == 0:
                with torch.no_grad():
                    val_attrid = torch.tensor([i for i in range(opts.attr_channel)]).to(
                        device
                    )
                    ...
                    for val_idx, val_batch in enumerate(test_dataloader):
                        ...
                        img_sample = torch.cat(
                            (val_img_A.data, val_fake_B.data, val_img_B.data), -2
                        )
                        save_file = os.path.join(
...
def test_one_epoch(
    ...
    with torch.no_grad():
        ...
        for test_idx, test_batch in enumerate(test_dataloader):
            ...
            test_attr_A_intensity = test_attr_A_intensity.view(
                test_attr_A_intensity.size(0), test_attr_A_intensity.size(2)
            )  # noqa
            test_attr_A_intensity = torch.sigmoid(
                3 * test_attr_A_intensity
            )  # convert to [0, 1]
            ...
            img_sample = torch.cat(
                (test_img_A.data, test_fake_B.data, test_img_B.data), -2
            )
            save_file = os.path.join(
                results_dir, f"test_{test_epoch}_idx_{test_idx}.png"
            )
            save_image(img_sample, save_file, nrow=52, normalize=True)

        test_l1_loss = test_l1_loss / len(test_dataloader)
        test_msg = (
            f"Epoch: {test_epoch}/{opts.n_epochs}, " f"L1: {test_l1_loss.item(): .6f}"
        )
        print(test_msg)
...
def interp(opts):
    ...
    assert (
        opts.test_epoch > 0 and opts.test_epoch % opts.check_freq == 0
    ), "Please choose correct test epoch"
    ...
    attribute_embed_file = os.path.join(
        checkpoint_dir, f"attribute_embed_{opts.test_epoch}.pth"
    )
    attr_unsuper_file = os.path.join(
        checkpoint_dir, f"attr_unsuper_embed_{opts.test_epoch}.pth"
    )
    ...
    with torch.no_grad():
        ...
        for test_idx, test_batch in enumerate(test_dataloader):
            ...
            for alpha in range(opts.interp_cnt):
                ...
                test_fake_B_alpha, _ = generator(
                    test_img_A.clone().detach(),
                    test_styles_A.clone().detach(),
                    test_alpha_intesnsity,
                    test_alpha_attr,
                )
            ...
            save_file = os.path.join(
                results_dir, f"interp_batch_{opts.test_epoch}_idx_{test_idx}.png"
            )
            ...
            save_file_sp = os.path.join(
                results_dir,
                f"all_char_same_specific_attr_random_{opts.test_epoch}_idx_{test_idx}.png",
            )  # noqa
            save_image(
                img_sample_random_attr, save_file_sp, nrow=52, normalize=True, padding=0
            )
            ...
            for attr_idx in range(opts.attr_channel):
                ...
                save_file_sp = os.path.join(
                    results_dir,
                    f"specific_attr_source_{attr_idx}_{opts.test_epoch}_idx_{test_idx}.png",
                )  # noqa
                save_image(
            ...
            for attr_idx in range(opts.attr_channel):
                ...
                save_file_sp = os.path.join(
                    results_dir,
                    f"specific_attr_target_{attr_idx}_{opts.test_epoch}_idx_{test_idx}.png",
                )  # noqa
                save_image(
                    img_sample_specific_attr,
                    save_file_sp,
